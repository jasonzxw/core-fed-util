 ...
 * @returns
 */
export function flattenJsonObject(obj, prefix = '', result = {}) {
 ...
 * @returns
 */
export function unFlattenJsonObject(obj, separator = '.') {
    const result = {};
    for (const key in obj) {
 ...
 */
export function getJsonObjectDiff(obj1, obj2) {
    const diff = {};
 ...
 * @returns
 */
export function renameJsonObject(obj, renameMap) {
    const renamedObj = {};
    for (const key in obj) {
 ...
 * @returns
 */
export function jsonToQueryString(obj, prefix = '') {
    const parts = [];
    for (const key in obj) {
        const value = obj[key];
