 ...
 * @returns {boolean}
 */
export function containSpecificEl(el:HTMLElement, child: HTMLElement): boolean {
    if (el && child) {
 ...
 * @returns {HTMLElement[]}
 */
export function findAllChildElements(el: HTMLElement): HTMLElement[] {
 ...
 * @returns {boolean}
 */
export function hasChildElements(el: HTMLElement): boolean {
    if (!el || !(el instanceof HTMLElement) || !el.children) {
        return false;
    }
